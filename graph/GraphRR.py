import numpy as np
from graph.GraphAPSP import GraphAPSP
...
class GraphRR(GraphAPSP):
    ...
    def bfs_truncated(self, source, dist):
        ...
        dist = np.array(dist)
        if (
            dist[self.vertex == source, self.vertex == v]
            <= dist[self.vertex == source, self.vertex == u] + w_uv
        ):
            return dist

        dist[self.vertex == source, self.vertex == v] = (
            dist[self.vertex == source, self.vertex == v] + w_uv
        )
        ...
        while len(PQ) > 0:
            ...
            dist[self.vertex == source, self.vertex == y] = (
                dist[self.vertex == source, self.vertex == u]
                + w_uv
                + dist[self.vertex == v, self.vertex == y]
            )
            for w in self.target[self.source == y]:
                if (
                    (w not in vis or not vis[w])
                    and dist[self.vertex == source, self.vertex == w]
                    > dist[self.vertex == source, self.vertex == u]
                    + w_uv
                    + dist[self.vertex == v, self.vertex == w]
                ):
                    vis[w] = True
                    PQ = np.append(PQ, [w])
    ...
    def find_source_affected(self, dist):
        ...
        dist[self.vertex == u, self.vertex == v] = (
            dist[self.vertex == u, self.vertex == v] + w_uv
        )
        ...
        while len(PQ) > 0:
            ...
            for z in self.source[self.target == x]:
                if (z not in vis or not vis[z]) and dist[
                    self.vertex == z, self.vertex == v
