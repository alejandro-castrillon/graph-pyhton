...
class GraphKNNB(GraphRR):
    ...
    def knnb_node_incremental(self, dist):
        ...
        for k_in in T1:
            dist[self.vertex == k_in, self.vertex == z] = self.get_weight(
                k_in, z
            )

        for k_out in T2:
            dist[self.vertex == z, self.vertex == k_out] = self.get_weight(
                z, k_out
            )
        ...
        for v in self.vertex:
            ...
            for k_in in T1:
                # if v == k_in:
                #    continue
                L_vz = dist[self.vertex == v, self.vertex == k_in][
                    0
            ...
            for k_out in T2:
                ...
                L_zv = dist[self.vertex == k_out, self.vertex == v][
                    0
