...
class DynamicGraph(Graph):
    ...
    def dynamic_decreasing_vertex(self, source, target):
        ...
        index = np.where(
            np.logical_and(self.source == source, self.target == target)
        )[0][0]
        returned = np.array([])
    ...
    def dynamic_incremental_random_vertex(
        self, weights=[1, 2, 3, 4, 5, 6, 7, 8, 9]
    ):
        ...
        index_for_target = np.invert(
            np.logical_or(
                np.in1d(self.vertex, self.target[node == self.source]),
                self.vertex == node,
            )
        )
        ...
        if choisen.size < 1:
            index_for_source = np.invert(
                np.logical_or(
                    np.in1d(self.vertex, self.source[node == self.target]),
                    self.vertex == node,
                )
            )
    ...
    def dynamic_incremental_node(
        ...
        self.source = np.concatenate(
            (self.source, sources, np.full(targets.size, node))
        )
        self.target = np.concatenate(
            (self.target, np.full(sources.size, node), targets)
        )
    ...
    def vertex_update(self, source, target, weight=1):
        ...
        if not self.directed:
            self.weight[
                np.logical_and(self.source == target, self.target == source)
            ] = weight
